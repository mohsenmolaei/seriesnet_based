...
class InputAttentionEncoder(nn.Module):
    ...
    def forward(self, inputs):
        ...
        for t in range(self.T):
            ...
            x = self.W_e(h_tm1).unsqueeze_(1).repeat(1, self.N, 1)
...
class TemporalAttentionDecoder(nn.Module):
    def __init__(self, M, P, T, stateful=False):
        ...
        self.v_y = nn.Linear(self.P, self.T )
    ...
    def forward(self, encoded_inputs, y):
        ...
        for t in range(self.T):
            ...
            x1 = self.W_d(d_tm1).unsqueeze_(1).repeat(1, encoded_inputs.shape[1], 1)
...
class DAGRU(nn.Module):
    def __init__(self, N, M, P, T, stateful_encoder=False, stateful_decoder=False):
        ...
        self.hsam= HSAM.HSAM(M)
    def forward(self, X_history, y_history):
        
        outEncoder = self.encoder(X_history)
        out = self.decoder(outEncoder*self.hsam(outEncoder), y_history)
        return out